...
var _xWars = __webpack_require__(2);
  ...
  _createClass(XWars, [{
    ...
    value: function action() {
      ...
      if (actions.indexOf(actions[0]) !== -1) {
        ...
        switch (_action[0]) {
          case 'go':
            state.update(_seamlessImmutable2.default.merge(current, {
              player: {
                time: current.player.time - 1,
                location: current.locations[_action[1]].name
              } }, {
              deep: true
            }));
            break;
          ...
          case 'redo':
            ...
            break;
          case 'reset':
            STATE.set(this, new _state2.default(_action[1]));
            break;
          default:
            break;
        }
    ...
    key: 'options',
    get: function get() {
      var state = STATE.get(this).current.asMutable({ deep: true });
      var options = {
        player: state.player,
        live: state.player.time > 0,
        commands: state.player.time > 0 ? state.locations.map(function (data, index) {
          return {
            name: 'go',
            value: index,
            data: data
      ...
      ACTIONS.set(this, options.commands.map(function (c) {
        return c.name;
      }));
      return options;
    }
...
var State = function () {
  ...
  _createClass(State, [{
    ...
    value: function undo() {
      if (this.index > 0) {
        this.index -= 1;
        this.states[this.index] = mergeState(this.current, this.index, this.states.length);
        return this.current;
      }
      return null;
    }
    ...
    value: function reset() {
      ...
      if (state || !this.states) {
        this.states = [(0, _configure2.default)(state)];
      } else {
...
function configureTime(state) {
  if (isNaN(+state.time) || +state.time < 1) {
    state.time = _defaults2.default.time;
  }
...
function configureLocations(state) {
  if (!Array.isArray(state.locations)) {
    state.locations = _defaults2.default.locations;
  }
  for (var i = state.locations.length - 1; i >= 0; i--) {
    var name = state.locations[i].name;
    if (typeof name !== 'string' || !name) {
      state.locations.splice(i, 1);
    }
  }
  while (state.locations.length < 2) {
    state.locations.push(_defaults2.default.locations[state.locations.length]);
  }
}
...
function configurePlayerLocation(state) {
  ...
  var random = Math.floor(Math.random() * state.locations.length);
...
function configurePlayer(state) {
  state.player = (0, _lodash2.default)(state.player) ? state.player : {};
...
function configure() {
  ...
  return (0, _seamlessImmutable2.default)(state || _defaults2.default);
}
