...
function pay(_ref) {
  ...
  var command = (0, _lodash2.default)(commands, { name: 'pay' });
  if (command) {
    if ((0, _lodash4.default)(+value)) {
      var amount = Math.max(0, Math.min(command.max, +value));
      var _current$player = current.player,
          money = _current$player.money,
          loan = _current$player.loan;
...
function borrow(_ref2) {
  var current = _ref2.current,
      commands = _ref2.commands,
      value = _ref2.value,
      interest = _ref2.interest;

  var command = (0, _lodash2.default)(commands, { name: 'borrow' });
  if (command) {
    if ((0, _lodash4.default)(+value)) {
      var amount = Math.max(0, +value);
      var _current$player2 = current.player,
          money = _current$player2.money,
          loan = _current$player2.loan;
      ...
      var player = {
        ...
        loan: {
          ...
          amount: loan.amount + amount,
          interest: (0, _lodash4.default)(interest) ? interest : loan.interest
        }
      };
      return _seamlessImmutable2.default.merge(current, { player: player }, { deep: true });
    }
...
var Advance = function () {
  ...
  _createClass(Advance, [{
    ...
    value: function isLocationOk(location) {
      return !!(0, _lodash2.default)(this.commands, { name: 'go', value: location });
    }
  }, {
    key: 'updatePlayer',
    ...
    value: function updateLoan() {
      var loan = this.state.current.player.loan.asMutable();
...
var Options = function () {
  ...
  _createClass(Options, [{
    ...
    value: function getOptions() {
      var info = new _optionsInfo2.default(this.state).info;
      var commands = new _optionsCommands2.default(this.state).commands;
      return { info: info, commands: commands };
    }
  }]);
...
var OptionsInfo = function () {
  function OptionsInfo(state) {
    _classCallCheck(this, OptionsInfo);

    this.state = state;
    this.info = this.getInfo();
  }

  _createClass(OptionsInfo, [{
    key: "getInfo",
...
var OptionsCommands = function () {
  function OptionsCommands(state) {
    _classCallCheck(this, OptionsCommands);

    this.state = state;
    this.commands = this.getCommands();
  }

  _createClass(OptionsCommands, [{
    key: 'getCommands',
    value: function getCommands() {
      var isLive = this.state.current.player.time > 0;
      var canRelocate = this.state.current.player.time > 1;
      var base = this.getBaseCommands();
      var locations = canRelocate ? this.getLocationCommands() : [];
      var items = isLive ? this.getItemCommands() : [];
      var loan = isLive ? this.getLoanCommands() : [];
    ...
    key: 'getLoanCommands',
    value: function getLoanCommands() {
      var _state$current$player = this.state.current.player,
          loan = _state$current$player.loan,
          money = _state$current$player.money;
...
exports.default = OptionsCommands;
